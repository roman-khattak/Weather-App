        ...
        Expanded(
          child: Obx(() {
            if (controller.isLoading.value) {
              ...
              return Center(child: Text('Enter a city to get the forecast.',style: TextStyle(color: Colors.white),));
            } else {
              return Column(
                children: [

                  Text("Today", style: GoogleFonts.macondo(color: Colors.white, fontSize: 20),),

                  if (controller.currentDayWeatherData.isNotEmpty) ...[

                 WeatherCard(weatherData: controller.currentDayWeatherData[0]),
                    ...
                    Expanded(
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          final weather = controller.otherDaysWeatherData[index];
                          return ListTile(
                            title: Text('${weather.temperature.toStringAsFixed(2)}°C',style: GoogleFonts.rubik(color: Colors.white, fontSize: 25),),
                            subtitle: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(weather.description,style: GoogleFonts.poppins(color: Colors.grey[400], fontSize: 16, ),),
                                Text(
                                  DateFormat('yyyy-MM-dd').format(weather.date.toLocal()),
